...
// Get all posts - SIMPLIFIED QUERY FOR DEBUGGING
...
*[_type == "post"] | order(publishedAt desc, _createdAt desc) {
  ...
  mainImage {
    ...
    asset-> { // Ensure we fetch the asset URL
      _id,
      url
    }
  },
...
// New query for searching posts
export const getPostsBySearchQuery = groq`
  *[_type == "post" && defined(slug.current) && (title match $query || excerpt match $query)] | order(_createdAt desc) {
    _id,
    title,
    "slug": slug.current,
    excerpt,
...
export async function fetchPostsBySearchQuery(query) {
  if (client) {
    return (
      (await client.fetch(getPostsBySearchQuery, { query: `*${query}*` })) || []
    );
  }
  return [];
}
